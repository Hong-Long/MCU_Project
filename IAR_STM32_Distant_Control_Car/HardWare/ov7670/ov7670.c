...
u8 OV7670_Init(void)
{
  ...
  // SCCB Register Reset位置一复位SCCB
  delay_ms(50); 
  //读取产品型号
  temp=SCCB_RD_Reg(0x0b); //读取设备ID的低8位  Product ID Number LSB（Read only）  
  printf("temp = %d\n",temp);
  if(temp!=0x73)return 2; 
  temp=SCCB_RD_Reg(0x0a); //读取设备ID的高8位  Product ID Number LSB（Read only）   
